...
public class RabinKarpSearch {
	...
	public static int patternSearch(char[] text, char[] pattern) {
		...
		for (int i = 0; i < n-(m-1); i++) {
			...
			if (patternHash == textHash && isEqual(text, i, i+m-1, pattern, 0, m-1)) {
				return i;
			}
			if (i < n-1-(m-1)) {
	...
	private static long calculateHash(char[] str, int end) {
		long hash = 0;
		for (int i = 0; i <= end; i++) hash += str[i] * Math.pow(prime, i);
		return hash;
	}

	private static boolean isEqual(char str1[], int start1, int end1, char str2[], int start2, int end2) {
